...
module tb();
    ...
    initial begin
        ...
        // Case 1: mfhi R5 -> Ra=R5(0101). Hex: 02800000
        dp.ram.memory[0] = 32'h02800000; 
        
        // Case 2: mflo R1 -> Ra=R1(0001). Hex: 00800000
        dp.ram.memory[4] = 32'h00800000; 
        ...
        $display("\n");
        $display("mfhi/mflo:");
        $display("Case 1 (mfhi R5): HI preloaded with 0x12345678");
        $display("R5 Final Value = %h (Expected: 12345678)", dp.R5.q);
        $display("Case 2 (mflo R1): LO preloaded with 0x87654321");
        $display("R1 Final Value = %h (Expected: 87654321)", dp.R1.q);
        $display("\n");
        $finish; 
    end
    ...
    always @(negedge clock) begin
        case (Present_state)
            Default: if (!clear) begin 
                Present_state <= C1_T0; 
                dp.PC.pc_q <= 32'h0;
                dp.HI.q <= 32'h12345678; // Preload HI with dummy data
                dp.LO.q <= 32'h87654321; // Preload LO with dummy data
                dp.R5.q <= 32'h0;
                dp.R1.q <= 32'h0;
            end
            ...
            C1_Hold: Present_state <= C2_T0; // Let PC naturally flow into the Case 2 fetch cycle!
    ...
    always @(Present_state) begin
        case (Present_state)
            ...
            // MFHI EXECUTION
            C1_T3: begin // HIout, Gra, Rin 
                RoutHI[0] <= 1; Gra <= 1; GPR_Rin <= 1; 
                #20 RoutHI[0] <= 0; Gra <= 0; GPR_Rin <= 0;
            end

            // MFLO EXECUTION
            C2_T3: begin // LOout, Gra, Rin
                RoutHI[1] <= 1; Gra <= 1; GPR_Rin <= 1; 
                #20 RoutHI[1] <= 0; Gra <= 0; GPR_Rin <= 0;
            end
        endcase
    end
endmodule