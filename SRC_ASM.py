...
register_mapping = {
    "t0": 0, "at": 1, "t1": 2, "t2": 3, "t3": 4, "t4": 5, "t5": 6, "t6": 7,
    "ra": 8, "sp": 9, "a0": 10, "a1": 11, "a2": 12, "a3": 13, "v0": 14, "v1": 15,
...
def parse_line(line: str, labels: dict[str, int], instruction_number: int) -> int:
    ...
    if instruction_name in reg_reg_reg:
        ...
        match_dict = match.groupdict()
        ra = get_register_number(match_dict['ra'])
        c2 = get_c2_for_branch(instruction_name)
        const = get_branch_offset(match_dict['const'], labels, instruction_number)
...
def second_pass(labels: dict[str, int], instructions: list[tuple[int, str]]) -> list[tuple[int, int]]:
    ...
    for memory_address, instruction in instructions:
        if match := re.match(r"^\s*word\s+(?P<const>\w+)\s*$", instruction.lower()):
            constant = get_constant(match.groupdict()['const'], labels, bit_width=32)
...
def main():
    parser = argparse.ArgumentParser(
        ...
        description='Takes an assembly file and produces a hex dump of the encoded instructions',
        epilog='Contact asmhelp@davlaf.com if you have questions or to report bugs')
